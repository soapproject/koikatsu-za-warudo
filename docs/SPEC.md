...
| 4e2 | Neck-look component disable | `chaCtrl.neckLookCtrl.enabled = false` per subject (cached + restored). The Unity Behaviour stops getting LateUpdate called → no neck/head bone writes. Earlier attempts via `neckLookScript.skipCalc = true` and head bone localRotation pinning both proved no-ops in playtest — `skipCalc` only changes the lerp factor to 1, doesn't skip rotation. See SKILL for the gory details. |
| 4e3 | KK in-game camera-tracking toggles | `Manager.Config.EtcData.FemaleEyesCamera/NeckCamera/EyesCamera1/NeckCamera1` (the same flags exposed in the game's config UI) cached and forced false. These gate `SetEyeNeckPtn(1000/1001)` (the camera-tracking branch) inside `HMotionEyeNeckFemale.Proc`. Belt-and-braces alongside the existing prefix and per-instance skipCalc. Restored on resume so the user's in-game preference comes back. |
